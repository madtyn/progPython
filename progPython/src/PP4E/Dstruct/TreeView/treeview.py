...
from tkinter import Tk, Entry, Button, Radiobutton, StringVar
from tkinter import LEFT, RIGHT, YES, X, END
from .treeview_wrappers import TreeWrapper, TreeViewer
from PP4E.Dstruct.Classics import btree
from PP4E.Lang.Parser import parser2
...
def test1_parser():
    test_parser("1 + 3 * (2 * 3 + 4)")


def test2_parser():
    test_parser("set temp 1 + 3 * 2 * 3 + 4")


def test3_parser():
    test_parser("set result temp + ((1 + 3) * 2) * (3 + 4)")
...
if __name__ == '__main__':
    ...
    def onRadio():
        if var.get() == 'btree':
            ...
            for btn in p_btns:
                btn.pack_forget()  # erase parser test buttons
            for btn in b_btns:
                btn.pack(side=LEFT)  # unhide binary buttons
        elif var.get() == 'ptree':
