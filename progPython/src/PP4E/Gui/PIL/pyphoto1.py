...
from tkinter import Tk, Canvas, Scrollbar, Button, Toplevel, PhotoImage
from tkinter import TOP, BOTH, YES, BOTTOM, RIGHT, LEFT, NW, X, Y
...
class ViewOne(Toplevel):
    ...
    def sizeToDisplaySide(self, scaler):
        ...
        if (newwide * newhigh < imgwide * imghigh):
            filter_ = Image.ANTIALIAS  # shrink: antialias
        else:  # grow: bicub sharper
            filter_ = Image.BICUBIC
        imgnew = imgpil.resize((newwide, newhigh), filter_)
    ...
    def zoom(self, factor):
        ...
        if factor < 1.0:  # antialias best if shrink
            filter_ = Image.ANTIALIAS  # also nearest, bilinear
        else:
            filter_ = Image.BICUBIC
        new = imgpil.resize((int(wide * factor), int(high * factor)), filter_)
        self.drawImage(new)
    ...
    def onZoomIn(self, event, incr=.10):
        ...
        :param incr: the zoom increase
        '''
        self.zoom(1.0 + incr)

    def onZoomOut(self, event, decr=.10):
        '''
        Triggered when zooming out
        :param event: the event which triggered this method
        :param incr: the zoom decrease
        '''
        self.zoom(1.0 - decr)
...
def viewThumbs(imgDir, kind=Toplevel, numcols=None, height=400, width=500):
    ...
    quit_ = Button(win, text='Quit', command=win.quit, bg='beige')
    ...
    while thumbs:
        ...
        for (imgfile, imgobj) in thumbsrow:
            ...
            def saveHandler(savefile=imgfile):
                '''
                Handles an imgfile to save
                :param savefile: the imgfile to save
                '''
                ViewOne(imgdir, savefile)
            link.config(command=saveHandler, width=linksize, height=linksize)
