...
def guessLocation(file, isOnWindows=(sys.platform[:3] == 'win'), trace=True):
    ...
    for directory in tries:
        if trace:
            print('Searching for %s in %s' % (file, directory))
        try:
            match = findFirst(directory, file)
        except OSError:
            if trace:
                print('Error while searching', directory)  # skip bad drives
        else:
            if match:
                return match
    if trace:
...
def runCommandLine(pypath, exdir, command, isOnWindows=False, trace=True):
    ...
    if isOnWindows:
        os.spawnv(os.P_DETACH, pypath, ('python', command))  # @UndefinedVariable pylint: disable=no-member
    else:
...
def launchBookExamples(commandsToStart, trace=True):
    ...
    assert pypath
    pydir, __ = os.path.split(pypath)  # up 1 from file, we discard the pyfile value
    if trace:
