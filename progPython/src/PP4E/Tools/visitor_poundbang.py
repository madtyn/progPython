...
changeToDefault = '#!\Python31\python.exe'  # used if no cmdline arg pylint: disable=anomalous-backslash-in-string


class PoundBangFixer(FileVisitor):
    '''
    Replaces directory paths in "#!" lines
    at the top of Unix scripts
    '''
    ...
    def visitfile(self, fullname):
        ...
        try:
            ...
            if (len(lines) > 0 and lines[0].startswith('#!')  # or lines[0][0:2] == '#!'
                    and 'python' in lines[0]):  # or lines[0].find() != -1
...
if __name__ == '__main__':
    ...
    LIST_ONLY = len(sys.argv) > 3
    walker = PoundBangFixer(changeto, LIST_ONLY)
    walker.run(rootdir)
    print('Visited %d files and %d dirs,' % (walker.fcount, walker.dcount), end=' ')
    print('found' if LIST_ONLY else 'changed', len(walker.clist), 'files')
    for fname in walker.clist:
        print(fname)
