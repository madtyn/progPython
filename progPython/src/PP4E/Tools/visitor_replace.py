...
class ReplaceVisitor(SearchVisitor):
    ...
    def __init__(self, fromStr, toStr, listOnly=False, trace=0):
        '''
        Builds this ReplaceVisitor
        :param fromStr: original string
        :param toStr: replacement string
        :param listOnly: if True, it will list and not replace
        :param trace: the tracing level
        '''
        self.changed = []
        self.toStr = toStr
...
if __name__ == '__main__':
    ...
    if listonly or input('Proceed with changes?') == 'y':
        ...
        for fileName in visitor.changed:
            print(fileName)
