...
from tkinter import Frame, Listbox, StringVar, Toplevel, Scrollbar  # Tk widgets
from tkinter import TOP, LEFT, RIGHT, BOTTOM, END, SUNKEN, SINGLE, BOTH, YES
from PP4E.Gui.Tools.widgets import frame, label, button, entry  # widget builders
from PP4E.Gui.Tools.guimixin import GuiMixin  # common methods
...
class FormGui(GuiMixin, Frame):
    ...
    def onNew(self):
        if not self.currform:
            return  # clear input form and key
        self.keytext.set('?%d' % len(self.index))  # default key unless typed
        for (__, text) in self.currslots:  # clear key/fields for entry
            text.set('')
    ...
    def onDelete(self):
        if not self.currform or not self.index:
            return
    ...
    def onList(self, evnt):
        if not self.index:
            return  # on listbox double-click
...
if __name__ == '__main__':
    from PP4E.Dbase.testdata import cast  # self-test code
    for k in cast.keys():
        print(k, cast[k])  # view in-memory dict-of-dicts
    FormGui(cast).mainloop()
    for k in cast.keys():
        print(k, cast[k])  # show modified table on exit
