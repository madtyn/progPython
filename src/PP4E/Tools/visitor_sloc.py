...
class LinesByType(FileVisitor):
    ...
    def visitsource(self, fpath, ext):
        '''
        Defines the implementation of the task to do when visiting a source code file.
        In this case, it simply counts
...
class PyLines(LinesByType):
    '''
    Subclass for only Python source code files
    '''
...
class SourceLines(LinesByType):
    '''
    Subclass for source code files from the restricted file extensions
    '''
