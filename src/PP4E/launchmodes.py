...
PY_PATH = sys.executable  # use sys in newer pys
...
class LaunchMode:
    ...
    def announce(self, text):  # @DontTrace pylint: disable=no-self-use
        '''
    ...
    def run(self, cmdline):  # @UnusedVariable pylint: disable=unused-argument, no-self-use
        '''
        Runs the cmdline
        :param cmdline:
        '''
...
class Popen(LaunchMode):
    ...
    def run(self, cmdline):
        cmdline = fixWindowsPath(cmdline)
        os.popen(PY_PATH + ' ' + cmdline)  # assume nothing to be read
...
class Fork(LaunchMode):
    ...
    def run(self, cmdline):
        ...
        if os.fork() == 0:  # start new child process
            os.execvp(PY_PATH, [PYFILE] + cmdline)  # run new program in child
...
class Start(LaunchMode):
    ...
    def run(self, cmdline):
        ...
        cmdline = fixWindowsPath(cmdline)
        os.startfile(cmdline)  # @UndefinedVariable pylint: disable=no-member
...
class Spawn(LaunchMode):
    ...
    """
    def run(self, cmdline):
        os.spawnv(os.P_DETACH, PY_PATH, (PYFILE, cmdline))  # @UndefinedVariable pylint: disable=no-member
...
class Top_level(LaunchMode):
    ...
    def run(self, cmdline):  # @UnusedVariable @DontTrace
...
class QuietPortableLauncher(PortableLauncher):
    '''
    Portable launcher without announcing
    '''
...
def selftest():
    ...
    '''
    file = 'echo.py'
    input('default mode... Press Enter.')
